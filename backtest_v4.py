...
OUTPUT_FILE   = "backtest_v4_results.csv"
ET            = pytz.timezone("America/New_York")

WING_WIDTH     = 40     # pts each side for IBF wings
MIN_SCORE      = 70     # GO threshold
...
MIN_VP             = 1.40       # minimum VIX/RV ratio — below this, realized vol is too close to implied
...
def score_entry(rv, vix, vix9d, range_pct, vwap_label, ts_label, ts_ratio,
    ...
    # Vol premium — IV (VIX proxy) / RV
    vp = (vix / rv) if vix and rv else None
    if vp:
        s["vol_premium"] = 30 if vp >= 1.5 else (20 if vp >= 1.2 else (10 if vp >= 1.0 else 3))
    else:
        s["vol_premium"] = 0
...
def run_backtest():
    ...
    for day in days:
        ...
        # ── Entry gate ────────────────────────────────────────────────────────
        rec = "GO" if score >= MIN_SCORE else ("WAIT" if score >= 45 else "SKIP")
