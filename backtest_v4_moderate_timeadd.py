...
Rules:
  ...
  - Add rule:    Two paths to add tranches (max 3 positions/day):
                 PATH 1 (DRIFT): SPX drifts 15+ pts AND slopes OK → new IBF at current SPX
                 PATH 2 (TIME):  90 min after entry, no drift add yet, P1 >= 10% profit,
                                 slopes OK → add at SAME center strike (thesis = pinning)
                 Tranche 3 is always drift-only.
...
OUTPUT_FILE   = "backtest_v4_moderate_timeadd_results.csv"
...
# Time-based add: if SPX stays within a tight range around center for N minutes
# after entry (no drift add triggered), add tranche 2 at SAME center strike.
# Thesis: confirmed pinning — butterfly is in its sweet spot.
TIME_ADD_MINUTES   = 45     # 10:45am (45 min after 10am entry)
TIME_ADD_MAX_RANGE = 10     # SPX must stay within ±10pts of center during window
    ...
    for day in days:
        ...
        if rec == "GO":
            ...
            if pos1["rr"] < MIN_RR:
                ...
                # Two paths to add:
                #   1. DRIFT ADD: SPX drifts 15+ pts → new IBF at current SPX (new center)
                #   2. TIME ADD:  90 min pass, no drift add yet, P1 profitable → add at SAME center
                # Tranche 3 is always drift-only.
                adds_done      = 0
                last_center    = pos1["center"]
                time_add_done  = False    # only one time-based add allowed
                ...
                if outcome1 in ("TARGET", "TIME_STOP", "EXPIRY"):
                    for _, bar in bars_from_entry.iterrows():
                        ...
                        # ── PATH 2: Time-based add (same center) ──────────────
                        # Condition: 45 min elapsed, no drift add yet, SPX has
                        # stayed within ±10pts of center the entire time (confirmed
                        # pinning), slopes still OK.
                        if (not time_add_done
                            and adds_done == 0
                            and bar_t >= time_add_t):

                            # Range check: was SPX within ±10pts of center
                            # for the entire window since entry?
                            ...
                            if max_dev > TIME_ADD_MAX_RANGE:
                                time_add_done = True   # SPX moved too much — not pinning
                                continue
                            ...
                            # Same center, refresh IV and T
                            add_tried = True
                            time_add_done = True
                            T_add     = T_from_bar(bar_t)
                            iv_at_add = adj_iv(iv, spx_entry, spx_now)
                            pos_add   = price_ibf(pos1["center"], iv_at_add, T_add)
                            ...
                            adds_done += 1
                            # last_center stays the same — tranche 3 drift is from original center
