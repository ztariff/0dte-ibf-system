#!/usr/bin/env python3
"""
run_research_2024.py — Edge discovery on 2023-11 through 2024-12 data.

Discovery period: Nov 2023 – Dec 2024 (~290 trading days)
Holdout period:   Jan 2025 – Mar 2026 (will be used for OOS validation later)
...
Writes all output to: research_results_2024.txt
"""
...
RESULTS_FILE = os.path.join(_DIR, "research_results_2024.txt")

# Date window: discovery period only
...
def main():
    ...
    out.write("=" * 100)
    out.write(f"SPX 0DTE VOL-SELLING EDGE RESEARCH — DISCOVERY PERIOD")
    out.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    out.write(f"Discovery window: {DATE_START} to {DATE_END}")
    out.write(f"Dates with complete data: {len(dates)} ({dates[0]} to {dates[-1]})")
    out.write(f"Total dates in universe: {len(all_dates)} (2025+ reserved for OOS validation)")
    out.write("=" * 100)

    if len(dates) < 200:
        out.write(f"\nWARNING: Only {len(dates)} dates available (expected ~290).")
...
1. DISCOVERY PERIOD ONLY: These results are from Nov 2023 – Dec 2024.
   Any edges found here MUST be validated on the 2025+ holdout data
   before being trusted. This is the training set, not the final answer.

2. BULL MARKET BIAS: This period is predominantly bull market with
   compressed VIX (mostly 12-22). Strategies favoring low-VIX, upward
   momentum, and narrow ranges may be over-represented. The 2025 holdout
   period includes some different conditions but still lacks a true bear
   market. Consider pulling 2022 data for stress testing.
...
NEXT STEPS:
  - Once 2025 data finishes pulling, run run_research.py (full dataset)
  - Top edges from this run become hypotheses to validate on 2025 data
  - Only edges that survive BOTH discovery AND holdout are tradeable
""")
